...
actions = {
    ...
    "simport": lambda _, n: Node("import",
        {"lrnode": _, "package": n[1]}, []),
    "suse": lambda _, n: Node("stdinc",
        {"lrnode": _, "stdlib": n[1]}, []),
    "spackage": lambda _, n: Node("package",
        {"lrnode": _, "name": n[1]}, []),
    "sfn": lambda _, n: Node("func",
        {"lrnode": _, "pkg": "", "clas": "", "name": n[1],
        "sargs": n[3], "rtype": n[6]}, list(n[7]).copy()),
    "spr": lambda _, n: Node("func",
        {"lrnode": _, "pkg": "", "clas": "", "name": n[1],
        "sargs": n[3], "rtype": "none"}, list(n[7]).copy()),
    ...
    #"stype": lambda _, n: Node("dtype",
    #                {"name": n[0]+("<"+(n[1][1] if n[1][1] else "")+">" if n[1] else "")})
}
